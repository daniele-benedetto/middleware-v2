  ...
  return (
    <div className="space-y-7">
      ...
      <section className="space-y-3">
        <Skeleton className="h-3.5 w-28 rounded-[6px] bg-card-hover" />

        <div className="grid grid-cols-(--grid-covers) gap-4">
          {Array.from({ length: cards }).map((_, index) => (
            <div
              key={`card-${index}`}
              className="space-y-3 rounded-[8px] border border-foreground bg-white px-5 py-4.5"
            >
              <Skeleton className="h-3.5 w-20 rounded-[6px] bg-card-hover" />
              <Skeleton className="h-6 w-3/4 rounded-[6px] bg-card-hover" />

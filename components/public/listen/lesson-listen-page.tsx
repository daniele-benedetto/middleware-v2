...
import { publicContentClassName as contentClassName } from "@/components/public/primitives";
...
export function LessonListenPage({ data, chunks }: LessonListenPageProps) {
  ...
  return (
    ...
    >
      <article>
        <div className="grid h-[calc(100svh-var(--public-header-height))] grid-rows-[auto_minmax(0,1fr)]">
          <PublicPageHero
            as="header"
            title={lesson.title}
          ...
          >
            <div className={`${contentClassName} h-full min-h-0 py-2 sm:py-8 lg:py-10`}>
              <LessonListenPlayer lesson={lesson} chunks={chunks} />
            </div>
          </section>
        </div>
        <LessonRelatedSection courseSlug={lesson.courseSlug} otherLessons={data.otherLessons} />
      </article>
    </main>
  );
}

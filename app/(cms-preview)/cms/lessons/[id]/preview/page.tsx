...
export default async function CmsLessonPreviewPage({ params }: CmsLessonPreviewPageProps) {
  ...
  const { lessonNumber, otherLessons, previousLesson, nextLesson } = getLessonPreviewContext(
    lesson,
    course,
  );
  ...
  return (
    <LessonLivePreviewPage
      ...
      editHref={`/cms/lessons/${id}/edit`}
      refreshHref={`/cms/lessons/${id}/preview`}
    />
  );
}
